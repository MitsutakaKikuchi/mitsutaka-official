...
- モーション演出:
  ...
  - 伝統 × 未来のモーショングラフィック（2026-09 第2弾・`src/scripts/motion.ts`）: 伝統の素材が未来の媒体へ移り変わる瞬間を一つずつ見せる
    - 開演（`Intro.astro` / `intro.ts`）: トップページ初回訪問時のみ、柝の一打が光の波紋になり、定式幕（黒・柿・萌葱）が引かれて走査線に溶ける。約1.9秒・クリック等でスキップ可。同じ閲覧中は再表示しない（sessionStorage `mk-intro`）。reduced-motion・クローラーでは出さない。`?intro=1` で強制表示（確認用）
    - 筆の一円相（`InkEnsou.astro` + `src/lib/brush.ts`）: 穂先の毛・かすれ・抜きまで描いた円相をビルド時に生成し、一筆で書かれるように描画。フッターはスクロールに合わせて筆が進む
    - 光の粒子の円相（`heroParticles.ts`）: WebGL の粒子が筆跡を追いかけて集まり、筆の書き順の方向へ流れ続ける。スクロールで火の粉のように舞い上がる。画面外では描画停止
    - 弦を走る光の信号: 光の三本弦に周期的な光のパルスが走り、通過点に三味線の文化譜（数字譜）の勘所数字が灯る。弦を弾くと、その位置の数字が浮かぶ
    - 墨 → 色（`[data-scan]`）: 写真は墨一色で現れ、光の走査線が通った所から色づく
    - 見出し（`[data-brush]` / `[data-scramble]`）: 見出しは筆で左から書かれるように現れ、欧文ラベルは文化譜の数字からデコードされる
    - 幕開き（`Curtain.astro` / `[data-curtain]`）: 出演依頼の夜の舞台に入ると定式幕が片側へ畳まれて開く
